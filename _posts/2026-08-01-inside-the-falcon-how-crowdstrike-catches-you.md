...
![image.png](/assets/img/csfalcon/image-3.png)
...
![image.png](/assets/img/csfalcon/image-4.png)

*the process-notify import we pivot on: PsSetCreateProcessNotifyRoutineEx.*

Cross Ref

![image.png](/assets/img/csfalcon/image-5.png)

*cross-references to PsSetCreateProcessNotifyRoutineEx where the callback gets registered.*

if not found will do a fallback on `PsSetCreateProcessNotifyRoutineEx`

![image.png](/assets/img/csfalcon/image-6.png)

*the driver resolves PsSetCreateProcessNotifyRoutineEx2 dynamically via MmGetSystemRoutineAddress, falling back to the Ex variant.*

anyway the Callback Functio register in n`sub_1400C3310` ⇒ CS_ProcessNotifyCallback

![image.png](/assets/img/csfalcon/image-7.png)

*the registered callback sub_1400C3310 (CS_ProcessNotifyCallback)  a thin wrapper into the real handler.*
...
*process-creation branch: non-NULL CreateInfo (a4) → sub_1400C39A8 allocates the internal tracking record.*
...
![*termination branch: CS_GetExistingProcessRecord marks the record terminated and records exit status/time.*](/assets/img/csfalcon/image-9.png)

*termination branch: CS_GetExistingProcessRecord marks the record terminated and records exit status/time.*
...
![*the kill mechanism: a negative NTSTATUS written to CreateInfo+64 (CreationStatus) blocks the process from ever starting.*](/assets/img/csfalcon/image-10.png)
...
*event dispatch: acquire context, compute event-type ID (1=create, 2=terminate), call the detection/dispatch path.*
...
![*inside CS_AllocProcessRecord: the call to sub_1400C42F4 (CS_GetProcessImagePath) that gathers the new process’s image path.*](/assets/img/csfalcon/image-12.png)

*inside CS_AllocProcessRecord: the call to sub_1400C42F4 (CS_GetProcessImagePath) that gathers the new process’s image path.*
...
![*PID 4 (the System process) short-circuits with STATUS_NOT_FOUND (0xC0000225) skipped entirely.*](/assets/img/csfalcon/image-13.png)

*PID 4 (the System process) short-circuits with STATUS_NOT_FOUND (0xC0000225) skipped entirely.*

now

![*CS_GetProcessImagePath: fast path grabs FileObject from CreateInfo; slow path calls CS_QueryImageName + CS_NormalizePath.*](/assets/img/csfalcon/image-14.png)

*CS_GetProcessImagePath: fast path grabs FileObject from CreateInfo; slow path calls CS_QueryImageName + CS_NormalizePath.*
...
![*the early gates: CS_IsSensorEnabled, g_DetectionEngine, g_SensorConfig+56, CS_AcquireEventBuffer any failure passes the process through unchecked.*](/assets/img/csfalcon/image-15.png)
...
![*the creation path: 552-byte alloc → CS_BuildCreationEvent → vtable selectors 72/6/10, then 5186 for full rule matching when hash and signature both resolved.*](/assets/img/csfalcon/image-16.png)
...
This is the function that actually *builds* the process-creation event handed to the detection engine. Section 1 saw `CS_DetectionDispatch` allocate 552 bytes and call this to fill it. Now we open it up.
...
![*CS_BuildCreationEvent wires the event as a COM-like object: 4 interface pointers + type ID 0x80002F3 ("process create").*](/assets/img/csfalcon/image-17.png)
...
![*refcount bump, then the core fields copied from the process record into the event: PID, parent PID, image path, command line, flags.*](/assets/img/csfalcon/image-18.png)
...
![*CS_GetProcessTokenInfo resolves the user SID / integrity level into +424 how the sensor knows who ran the process.*](/assets/img/csfalcon/image-19.png)

*CS_GetProcessTokenInfo resolves the user SID / integrity level into +424 how the sensor knows who ran the process.*

`CS_GetProcessTokenInfo` walks a rundown-protected token reference and resolves the user **SID / integrity level** into `+424`. That’s how the sensor knows *who* ran the process (SYSTEM vs normal user vs elevated). Then two vtable calls into the engine enrich signing info:
...
![*two engine vtable calls: selector 8 = signer/certificate chain, selector 9 = reputation/hash. Failure zeroes the field; event still ships.*](/assets/img/csfalcon/image-20.png)
...
![*file/transaction flags at +520: bit 1 = image launched inside a TxF transaction (process doppelgänging), bits 2/4 = FILE_OBJECT state.*](/assets/img/csfalcon/image-21.png)
...
![*final engine call stores a verdict/context handle at +528, then deep-copies the process short name so the event owns its own string.*](/assets/img/csfalcon/image-22.png)
...
`CS_DetectRemoteThread` compares the **thread’s owning process** against the **creator context**. A thread whose start address lands in a *different* process than the caller = classic `CreateRemoteThread` injection. Fires only when: not a system thread, has a resolved start address, and `byte_1402FB824` (config flag) is set.

![*CS_ThreadCreateCallback entry: PASSIVE_LEVEL / APC guards, then the thread-create vs thread-exit split.*](/assets/img/csfalcon/image-23.png)

*CS_ThreadCreateCallback entry: PASSIVE_LEVEL / APC guards, then the thread-create vs thread-exit split.*

![*CS_DetectRemoteThread: start address in a different process than the caller = classic CreateRemoteThread injection.*](/assets/img/csfalcon/image-24.png)

*CS_DetectRemoteThread: start address in a different process than the caller = classic CreateRemoteThread injection.*

![*the 360-byte thread event (‘Tes1’) built and submitted with ProcessId, ThreadId and the resolved start address.*](/assets/img/csfalcon/image-25.png)
...
- **PE header parsed in kernel** (`RtlImageNtHeader`) → characteristics/subsystem/machine become telemetry. Malformed PE = interesting fuzz target.
    ...
    ![*RtlImageNtHeader parses the PE in kernel; Characteristics / DllCharacteristics / Subsystem / Machine become telemetry (a fuzz surface).*](/assets/img/csfalcon/image-26.png)
    
    *RtlImageNtHeader parses the PE in kernel; Characteristics / DllCharacteristics / Subsystem / Machine become telemetry (a fuzz surface).*
    

![image.png](/assets/img/csfalcon/image-27.png)

![*kernel images take a first-class branch (CS_HandleDriverLoad) bring-your-own-vulnerable-driver visibility.*](/assets/img/csfalcon/image-28.png)
...
![*CS_CheckHandleAccess: process mask 0xFFEDCFFF / thread mask 0xFFEDF3FF decide “harmless”; VM_READ/WRITE, CREATE_THREAD, DUP_HANDLE trip the check. Self-access always allowed.*](/assets/img/csfalcon/image-29.png)

*CS_CheckHandleAccess: process mask 0xFFEDCFFF / thread mask 0xFFEDF3FF decide “harmless”; VM_READ/WRITE, CREATE_THREAD, DUP_HANDLE trip the check. Self-access always allowed.*
...
![*CS_RegistryCallback: kernel-origin ops ignored except RegNtPreDeleteValueKey (class 47); user-origin ops fully inspected.*](/assets/img/csfalcon/image-30.png)

*CS_RegistryCallback: kernel-origin ops ignored except RegNtPreDeleteValueKey (class 47); user-origin ops fully inspected.*

![*the recursion-guarded dispatch: KeSetKernelStackSwapEnable(0) pins the stack, guard blocks the sensor’s own registry access from re-entering.*](/assets/img/csfalcon/image-31.png)
...
![*CS_RegisterCallouts_Eng1/Eng2: near-identical loops registering callouts against the two WFP engine handles from a shared callout table.*](/assets/img/csfalcon/image-32.png)
...
![*the callout GUIDs (shared prefix 0x4838FD4EC5996155) a stable host/network fingerprint you can match in `netsh wfp show state`.*](/assets/img/csfalcon/image-33.png)
...
- **`FwpsFlowAssociateContext0`** attaches the record so **every packet/stream chunk on that connection is delivered to the callouts *with this record as context***  that’s what makes inspection *stateful*.
...
![*CS_FlowEstablishClassify allocates a per-flow record (pool tag ‘CSFR’) and stores the 5-tuple capped at 10,000 concurrent flows.*](/assets/img/csfalcon/image-34.png)

*CS_FlowEstablishClassify allocates a per-flow record (pool tag ‘CSFR’) and stores the 5-tuple capped at 10,000 concurrent flows.*

![*the FwpsFlowAssociateContext0 loop that attaches the record to every layer what makes packet inspection stateful per-connection.*](/assets/img/csfalcon/image-35.png)

*the FwpsFlowAssociateContext0 loop that attaches the record to every layer what makes packet inspection stateful per-connection.*
...
![*CS_InjectTransportPacket: copies payload into a ‘pPDd’ buffer, builds the MDL + NET_BUFFER_LIST for injection.*](/assets/img/csfalcon/image-36.png)

*CS_InjectTransportPacket: copies payload into a ‘pPDd’ buffer, builds the MDL + NET_BUFFER_LIST for injection.*

![*the protocol branch: proto == 6 (TCP) → FwpsStreamInjectAsync0, else FwpsInjectTransportSendAsync1 the connection-kill primitive.*](/assets/img/csfalcon/image-37.png)
...
![*CS_KillFlaggedFlows walks tracked flows in bounded batches and calls CS_ResetFlow → injects a TCP reset. This is how C2 gets killed.*](/assets/img/csfalcon/image-38.png)
...
![the minifilter registration, guarded by a state machine](/assets/img/csfalcon/image-39.png)
...
![12 IRP majors hooked  near-total file-system visibility](/assets/img/csfalcon/image-40.png)
...
![every FLT callback is a thin trace shim around the real handler  same pattern as the rest of the sensor](/assets/img/csfalcon/image-41.png)

every FLT callback is a thin trace shim around the real handler  same pattern as the rest of the sensor

The handler resolves the **normalized file path** (via `FLT_FILE_NAME_INFORMATION`), determines the *kind* of set-info (rename/delete/hardlink), packages it → shared event ring → engine → cloud.
...
cspcm4 uses a **named `Ex` callback object as a singleton rendezvous**. First CS module to init creates the object and publishes a broker callback; later modules just *notify* the existing object and exchange context pointers. `Pcm_BrokerCallback` (0x180001010) is trivial: `*Argument1 = qword_180005168`  whoever calls gets a pointer to the shared context. Classic way for a driver family to share one trusted state block without exporting it by symbol.

![named-callback singleton: first loader owns the context, others rendezvous](/assets/img/csfalcon/image-42.png)

named-callback singleton: first loader owns the context, others rendezvous

![the actual ‘pinned’ 4 KB code page being copied into resident context.](/assets/img/csfalcon/image-43.png)
...
![cspcm4’s inter-module callback registry  16 slots](/assets/img/csfalcon/image-44.png)

cspcm4’s inter-module callback registry  16 slots

![the sensor’s APC-injection primitive lives here.](/assets/img/csfalcon/image-45.png)
...
![embedded SHA-1 allowlist  data only, compared by the consumer, not here.](/assets/img/csfalcon/824c8fe2-fd0d-4cce-abdc-03cf8781b1ac.png)
...
![image.png](/assets/img/csfalcon/image-46.png)

![image.png](/assets/img/csfalcon/image-47.png)

![image.png](/assets/img/csfalcon/image-48.png)
...
![image.png](/assets/img/csfalcon/image-49.png)

![the DNS-monitoring subsystem (NetworkPolling::DnsResolver).](/assets/img/csfalcon/image-50.png)
...
![image.png](/assets/img/csfalcon/image-51.png)

![image.png](/assets/img/csfalcon/image-52.png)

![image.png](/assets/img/csfalcon/image-53.png)
...
![image.png](/assets/img/csfalcon/e422d1da-6055-477b-9dc0-736c0b854e1c.png)

![browser extension governance + credential-store monitoring.](/assets/img/csfalcon/image-54.png)
...
![Falcon Identity Threat Detection  the endpoint-side actors.](/assets/img/csfalcon/image-55.png)

Falcon Identity Threat Detection  the endpoint-side actors.

![the CsIntegration EndpointServer  the RPC handler implementation.](/assets/img/csfalcon/image-56.png)
...
![image.png](/assets/img/csfalcon/image-57.png)

![the CsInteg RPC interface registration.](/assets/img/csfalcon/image-58.png)

the CsInteg RPC interface registration.

![the per-call gate  checks transport auth level, **not caller identity** (authentication without authorization](/assets/img/csfalcon/image-59.png)
...
We’ve seen every place the sensor *sees* you. Now: when an event is built, how does it become a kill/allow decision?
...
![the object’s method table decoded.](/assets/img/csfalcon/image-61.png)
...
![where ‘is this malware?’ gets answered.](/assets/img/csfalcon/image-62.png)
...
![image.png](/assets/img/csfalcon/image-64.png)
...
| --- | --- | --- |
| `0x86968` | CS_ValidateCreatorThread | is the creating thread a legit parent, or a hijacked/remote thread? |
| `0x8696C` | CS_DetectInjection | full cross-process injection check |
...
![the injection-detection layer.](/assets/img/csfalcon/image-65.png)
...
![the detection engine is *installed at runtime*, not compiled in.](/assets/img/csfalcon/image-66.png)
...
- **The matcher is `engine_vtable[+16]`**  a function *inside the loaded content object*, not in csagent. The driver has no idea how matching works; it calls slot 16 and reads a verdict.
- Inputs are **file hash + signature + event context**.

![the actual malware decision  a vtable call into cloud-loaded content.](/assets/img/csfalcon/image-67.png)
...
| 7 | **Reset-based kill is racy** | `CS_KillFlaggedFlows` (W2/W3) | Termination is a TCP RST injected *after* a verdict. Short-lived flows (a quick beacon) may complete before the reset lands. Mitigation after detection, not prevention. |
...
| 0x1403622A0 | `CS_FltPreCreate` | pre IRP_MJ_CREATE |
